   ...
   data.append('since', 0);*/
		...
		value: function render() {
			if (!this.state.goods[0]) {
				...
				return React.createElement(Product, { id: this.state.goods[0].id, name: this.state.goods[0].name, img: this.state.goods[0].photos, price: this.state.goods[0].price, createdAt: this.state.goods[0].created_at, desc: this.state.goods[0].description, views: this.state.goods[0].views });
			}
		}
	}]);
...
var SearchGoods = function (_React$Component3) {
	_inherits(SearchGoods, _React$Component3);

	function SearchGoods(props) {
		_classCallCheck(this, SearchGoods);

		return _possibleConstructorReturn(this, (SearchGoods.__proto__ || Object.getPrototypeOf(SearchGoods)).call(this, props));
	}

	_createClass(SearchGoods, [{
		key: 'goToIndex',
		value: function goToIndex(event) {
		...
		value: function render() {
			return React.createElement(
				...
				React.createElement(
					'button',
					{ type: 'button', 'class': 'btn btn-primary', onClick: this.goToIndex, style: { display: 'inline' } },
					'\u041D\u0430\u0439\u0442\u0438!'
				)
			);
		}
...
ReactDOM.render(React.createElement(SearchGoods, null), document.getElementById('search-wrapper'));